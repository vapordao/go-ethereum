...
func (s *PublicBlockChainAPI) GetProof(ctx context.Context, address common.Address, storageKeys []string, blockNrOrHash rpc.BlockNumberOrHash) (*AccountResult, error) {
	...
	for i, key := range storageKeys {
		if storageTrie != nil {
			...
			storageProof[i] = StorageResult{key, state.GetState(address, common.HexToHash(key)).Bytes(), common.ToHexArray(proof)}
		} else {
			storageProof[i] = StorageResult{key, hexutil.Bytes{}, []string{}}
		}
	}
